  ...
  <div class="panel-body card ">
    <div class="col-md-121 slim-scroll" data-height="300">
      <header class="panel-heading" style="padding:10px 0px !important;">
        <header class="btn btn-default btn-sm"> @icon('solid/exclamation-circle') @langapp('drugtradeinfo') &nbsp;&nbsp;[ <span class="badge badge-warning mr-2">@langapp('drugtradename') </span> ]</header>
      </header>
      {!! Form::open(['route' => 'indexing.api.savedrugtradename', 'id' => 'createDrugTradeName', 'class' => 'bs-example form-horizontal m-b-none']) !!}
          ...
          <div class="col-lg-121">
            <div class="col-lg-6" >
              <div class="form-group" id="divtrname">
                ...
                <div class="col-lg1-2">
                  <div class="add_field_button btn btn-success"><i class="fa fa-plus-circle" aria-hidden="true"></i>
</div>
                </div>
              </div>
    ...
    <div class="panel-body card">
      <div class="form-group">
        <div class="drugtrade-list" id="nestable"> @widget('Indexing\ShowDrugTradeName', ['user_id' => \Auth::id(), 'jobid' => $jobdata->id, 'orderid' => $jobdata->orderid]) </div>
      </div>
    </div>
  </div>
</div>
...
function ajaxtradename(val){
   if(val !='') {
	...
	$("#ajaxterm-"+val).addClass("activeli");
		...
		axios.post('{{ get_option('site_url') }}api/v1/indexing/ajax/tradenamedata', {
			selectedterm: val
		})
		.then(function (response) {
			$('#ajaxdrugtradename').fadeIn();  
			$('#ajaxdrugtradename').html(response.data.htmldrugterm);
...
function selectedCountry(name){
	...
	$(add_button).click(function(e){ 
		...
		if(x < max_fields){
			x++; 
				$(wrapper).append('<div class="form-group" id="btn'+x+'"><div class="col-lg-6"><input type="text" class="form-control" id="txtdrugtradename" placeholder="Drug Trade Name" name="txtdrugtradename[]" value="{{ @$data['txtdrugtradename']}}" autocomplete="off" /></div><div class="col-lg1-2"><a href="#" id="'+x+'" class="remove_field btn btn-danger"><i class="fa fa-trash" aria-hidden="true"></i></a></div></div>'); 
		}
	});
