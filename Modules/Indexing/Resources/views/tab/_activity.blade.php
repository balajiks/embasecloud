    ...
    <aside>
        <section class="">
            ...
            <header class="header">
                ...
                <a href="{{ route('todo.create', ['module' => 'indexing', 'id' => $indexing->id]) }}" data-toggle="ajaxModal" class="btn btn-success btn-sm pull-right btn-icon" id="new-task" >
                    @icon('solid/plus')
                </a>

                <div class="m-t-sm pull-left text-muted">
                       <label >
                            <input id="checkAll" type="checkbox" data-id="{{ $indexing->id }}" name="indexing" {!! $indexing->todo_percent >= 100 ? 'checked="checked" disabled=""' : '' !!}>
