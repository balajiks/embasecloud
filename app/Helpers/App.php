...
function classByName($name)
{
    switch ($name) {
		case 'indexing':
            return new \Modules\Indexing\Entities\Indexing;
            break;
        case 'deals':
...
function renderAjaxcustomButtonSquare($text = 'save', $icon = 'fas fa-paper-plane', $overwriteText = false)
{
    return '<button type="submit" class="btn btn-' . get_option('theme_color') . ' formSaving submit" style="padding:5px 12px !important; top:5px !important;"><i class="' . $icon . '"></i> ' . langapp($text, [], app()->getLocale()) . '</button>';
}
